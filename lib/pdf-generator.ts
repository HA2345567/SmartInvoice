...
export class PremiumPDFGenerator {
  ...
  private renderUltraLuxuryLayout(data: InvoiceData) {
    ...
    // 1. Company Logo or Name
    let drawnLogo = false;
    if (data.companyLogo) {
      drawnLogo = this.drawLogo(data.companyLogo, leftMargin, 18, 50, 22);
    }
    ...
    if (!drawnLogo) {
      ...
      this.doc.text(data.companyName || 'SmartInvoice', leftMargin, 32);
    }

    // Address Block below Company Name / Logo
    this.doc.setFontSize(9);
    this.doc.setFont('helvetica', 'normal');
    this.doc.setTextColor(80, 80, 80);
    if (data.companyAddress) {
      this.doc.text(data.companyAddress, leftMargin, 46);
    }
  ...
  private renderMicrosoftLayout(data: InvoiceData) {
    ...
    let drawnLogo = false;
    if (data.companyLogo) {
      drawnLogo = this.drawLogo(data.companyLogo, this.margin, startY - 10, 45, 20);
    }
  ...
  private drawLogo(logoUrl: string | undefined, x: number, y: number, maxW: number = 40, maxH: number = 20): boolean {
    ...
    try {
      ...
      this.doc.addImage(logoUrl, format, x, y, maxW, maxH);
      return true;
    } catch (e) {
  ...
  private addFooterBranding(data: InvoiceData) {
    if (data.whiteLabelMode) {
      return; // White-label mode: Suppress "Generated by SmartInvoice" branding completely
    }
