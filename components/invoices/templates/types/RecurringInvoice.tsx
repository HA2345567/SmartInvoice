...
const RecurringInvoice: React.FC<Props> = ({ data, styleClass }) => {
    const styles = {
    ...
    return (
        <div className={styles.container}>
            <header className={styles.header}>
                ...
                <div className={styles.subtitle}>RECURRING INVOICE</div>
            </header>

            {data.recurring && (
                <div style={{ display: 'flex', gap: '20px', marginBottom: '20px', padding: '10px', background: '#f8f9fa' }}>
                    <div>
                        <b>Period:</b> {data.recurring.subscriptionPeriod}
                    </div>
                    <div>
                        <b>Next Billing:</b> {data.recurring.nextBillingDate}
                    </div>
                </div>
            )}
